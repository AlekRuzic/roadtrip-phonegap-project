...
function sendStartTripData() {
    ...
    if (startLat == '' || startLong == '' || startAlt == '' || tripDescription == '' || tripName == '') {
        ...
        firebase.database().ref('Team Data/users/' + userId +'/trips/' + tripName).set({
...
function sendEndTripData() {
    var userId = appAuth.getCurrentUser();
    firebase.database().ref("Team Data/users/" + userId + "/trips/" + currentTripName).update(
      {
...
function previousTrips() {
    ...
    var userId = appAuth.getCurrentUser();
    firebase.database().ref('Team Data/users/' + userId + "/trips")
...
var checkIfTripInProgress = function() {
    firebase
    .database()
    .ref("Team Data/users/" + firebase.auth().currentUser.uid + "/trips")
